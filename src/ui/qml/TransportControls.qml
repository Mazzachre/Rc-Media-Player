	...
	Column {
		...
		Slider {
			...
			MouseArea {
				...
				anchors.fill: parent
				hoverEnabled: true
				onClicked: Video.setPosition(Math.round((positionMouseArea.mouseX/position.width)*Video.duration));
			}
		...
		Item {
			...
			Row {
				...
				Button {
					...
					text: "\uE723"
					onClicked: player.previousMovie()//This should go directly to the socket...
				}
				...
				Button {
					...
					font.pixelSize: transportControls.height / 4
					text: "\uE722"
					onClicked: player.nextMovie()//This should go directly to the socket...
				}
				...
				Button {
					...
					font.family: icons.name
					font.pixelSize: transportControls.height / 4
					text: UiState.fullScreen ? "\uE6C9" : "\uE659"
					onClicked: UiState.toggleFullScreen()					
				}
			}
			...
			Row {
				...
				Slider {
					...
					value: Audio.volume
					onMoved: Audio.setVolume(volume.value)
